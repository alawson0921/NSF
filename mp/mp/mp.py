...
def long_nsfraw(nstep,base):
    mp1 = mp.mpf(base-nstep)
    mp2 = mp.mpf(base)
    while(mp1>0):
        mp2 = mp.mpf(mp2 * mp1)
        mp1 = mp.mpf(mp1-nstep)
    return mp2
"""
...
##finding the lawson constant at higher precision
def find_l():
    #x = np.arange(lawson_const+mp.mpf(0.0000000000000000000000000000000001),lawson_const+0.00000000000001,0.0000000000000000000000000000000001)
    x = []
    for a in range(mp.mpf(lawson_const+0.0000000000000000000000000000000001),lawson_const+0.00000000000001,0.0000000000000000000000000000000001):
        x.append(a)
    for y in x:
        if(long_nsfraw(0.001,y)>99999999999999999999999999999999999):
            print(mp.mpf(y-0.0000000000000000000000000000000001))
            return y-0.0000000000000000000000000000000001
